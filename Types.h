...
struct Node {
    ...
    double m_X2;
    double m_X3;
    //€вл€етс€ ли вершиной или внутренним узлом
    bool is_Vertex;
    ...
    friend std::ostream &operator<<(std::ostream &p_Out, const Node &p_Node) {
        ...
        //if (p_Node.is_Vertex) p_Out << "явл€етс€ вершиной  Ё" << std::endl;
        //else p_Out << "явл€етс€ внутренним узлом  Ё" << std::endl;
        return p_Out;
    }
};
...
struct BoundaryFiniteElement {
    ...
    friend std::ostream &operator<<(std::ostream &p_Out, const BoundaryFiniteElement &p_SFE) {
        p_Out << "ID поверхностного  Ё - " << p_SFE.m_ID << std::endl;
        p_Out << "ID поверхности - " << p_SFE.m_IDBoundary << std::endl;
        p_Out << "ID узлов поверхностного  Ё : ";
